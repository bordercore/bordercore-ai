"""
This module retrieves a user's upcoming calendar events and generates a prompt
to query an LLM about their schedule. It reads OAuth2 credentials from a file,
connects to the Google Calendar API, and formats calendar events into a prompt
for a chatbot model to answer user questions.
"""

import json
from datetime import datetime, timedelta
from pathlib import Path

import dateutil.parser
import httplib2
from apiclient.discovery import build
from oauth2client.client import OAuth2Credentials

import settings

from .rfc3339 import datetimetostr
from .rfc3339 import now as now_rfc3339


def get_schedule(command: str) -> str:
    """
    Build a schedule-aware prompt from the next 7 days of Google Calendar events.

    This function authenticates to Google Calendar using OAuth2 credentials loaded
    from `./calendar_credentials.json`, fetches the user’s upcoming events for the
    next week from `settings.calendar_id`, and returns a single formatted string
    suitable for sending to a language model. The prompt includes:
      • The weekday name for “today” (to anchor relative terms),
      • The caller’s question (`command`),
      • One line per event of the form:
        "One event is called <SUMMARY> and starts on <Weekday HH:MMAM/PM>"

    Args:
        command: A natural-language question about the user’s schedule
            (e.g., "Am I free Friday afternoon?" or "What’s first tomorrow?").

    Returns:
        A complete prompt string for an LLM to answer the question using only
        the embedded calendar context.
    """
    credentials_path = Path("./calendar_credentials.json")
    try:
        with credentials_path.open("r", encoding="utf-8") as file:
            cal_info = json.load(file)
    except FileNotFoundError as e:
        raise FileNotFoundError(f"Credential file not found: {credentials_path}") from e
    except json.JSONDecodeError as e:
        raise ValueError(f"Invalid JSON in credentials file: {e}") from e

    credentials = OAuth2Credentials(
        cal_info["access_token"],
        cal_info["client_id"],
        cal_info["client_secret"],
        cal_info["refresh_token"],
        cal_info["token_expiry"],
        cal_info["token_uri"],
        cal_info["user_agent"],
        cal_info["revoke_uri"],
        cal_info["id_token"],
        cal_info["token_response"],
    )

    http = httplib2.Http()
    http = credentials.authorize(http)
    service = build(
        serviceName="calendar",
        version="v3",
        http=http,
        developerKey="",
        cache_discovery=False
    )
    time_max = datetime.now() + timedelta(days=7)

    events = service.events().list(calendarId=settings.calendar_id,
                                   orderBy="startTime",
                                   singleEvents=True,
                                   timeMin=str(now_rfc3339()).replace(" ", "T"),
                                   timeMax=datetimetostr(time_max)).execute()

    prompt = f"""
    I will give you a series of events on my personal calendar. I want you to answer a question about my calendar based on those events. For reference assume today is {datetime.now().strftime("%A")}. The question is the following: {command}. Answer that question based on the following list of calendar events. If there are no events on my calendar, say some variation of the following: 'Your calendar is clear today'.
    """

    for event in events["items"]:
        date_key = "dateTime" if "dateTime" in event["start"] else "date"
        start_pretty = dateutil.parser.parse(event["start"][date_key]).strftime("%A %I:%M%p")
        prompt = prompt + f"""
        One event is called {event['summary']} and starts on {start_pretty}
        """

    if settings.debug:
        print(prompt)

    return prompt
